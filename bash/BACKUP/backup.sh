...
# 		if [ ! -e "$source" ]; then
# 			echo "Backup source/sources not exist" >&2
# 			return 1
# 		fi
# 	done	
#  }
...
# 	local archive_name="$(pwd)/backup_${timestamp}.tar.gz"

#   if check_backup_sorce "${files[@]}"; then
#       tar -czvf "$archive_name" "${files[@]}" 2>/dev/null
#       if [[ $? -eq 0 ]]; then
#           echo "$archive_name"
#       else
...
# 	local unit=$(echo "$value" | grep -o '[A-Za-z]*$' | tr '[:upper:]' '[:lower:]')
...
#  upload_to_drive() {
# 	local files="$1"
# 	local gdrive_free_space=$(rclone about gdrive: | awk -F ' ' 'NR == 3 {print $2 $3}')
# 	local files_size=$(du -h "$files" | awk -F ' ' '{print $1}')

# 	if (( $(convert_to_bytes "$gdrive_free_space") > $(convert_to_bytes "$files_size") )); then
# 		echo $(rclone copy "$files" gdrive:backup_folder)
# 		echo "Backup done!"
# 	else
# 		echo "Error: Files can't by uploaded" >&2
# 		return 1
# 	fi
#  }
...
# 	archive_files "$FILES"
# 	upload_to_drive $(archive_files "$FILES")
# else
# 	archive_files "$BACKUP_SOURCES"
# 	upload_to_drive $(archive_files "$BACKUP_SOURCES")
# fi
